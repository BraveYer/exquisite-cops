...
export default function LandingPage() {
  ...
  const [lastRedirectedMatch, setLastRedirectedMatch] = useState<string | null>(null);
  const MAX_PLAYERS = 1; 
  ...
  // 2. VERIFICĂ COADA (Pentru afișaj număr)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const matchCheck = setInterval(() => {
      fetch('/api/match/current', { cache: 'no-store' })
        .then(res => res.json())
        .then(data => {
          // Dacă a găsit un meci ȘI nu te-a mai teleportat deja la el sesiunea asta
          if (data.matchId && data.matchId !== lastRedirectedMatch) {
            setQueue(false); // Oprește butonul roșu
            setLastRedirectedMatch(data.matchId); // Ține minte meciul ca să nu te bage în buclă
            router.push(`/match/${data.matchId}`); // TELEPORT!
          }
        })
  ...
  }, [session, lastRedirectedMatch, router]);

  // FUNCȚIA DE START SEARCH (Declanșează animația instant)
  const toggleQueue = async () => {
    const newState = !inQueue;
    setQueue(newState); // Pornește animația imediat!

    const action = newState ? 'join' : 'leave';
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-cyan-500 pb-20">
      ...
      <main className="max-w-6xl mx-auto px-6 pt-16 text-center">
        {/* BANNER MECI ACTIV (Ideea ta!) */}
        {session && !inQueue && lastRedirectedMatch && (
          <div className="bg-gradient-to-r from-cyan-500/20 to-blue-600/20 border border-cyan-500/30 p-4 rounded-2xl flex flex-col sm:flex-row justify-between items-center backdrop-blur-md mb-8 shadow-[0_0_20px_rgba(6,182,212,0.15)] animate-fade-in-down">
            <div className="flex items-center gap-3 mb-3 sm:mb-0">
              <div className="w-2.5 h-2.5 bg-cyan-400 rounded-full animate-ping"></div>
              <p className="text-cyan-400 font-bold uppercase tracking-[0.2em] text-xs">Atenție: Ai un meci în desfășurare!</p>
            </div>
            <button 
              onClick={() => router.push(`/match/${lastRedirectedMatch}`)}
              className="bg-cyan-500 hover:bg-cyan-400 text-black px-6 py-2 rounded-xl font-black text-xs transition-all uppercase tracking-wider"
            >
              Întoarce-te la Meci
            </button>
          </div>
          ...
          <div className="max-w-3xl mx-auto mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <div className="bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-sm">
              ...
              <p className="text-5xl font-black text-white">
                {stats.wins + stats.losses > 0 ? Math.round((stats.wins / (stats.wins + stats.losses)) * 100) : 0}<span className="text-2xl text-gray-400">%</span>
              </p>
            </div>
          </div>
        )}
            ...
            <div className="flex flex-col items-center gap-4 w-full">
              ...
              {inQueue ? (
                <div className="animate-pulse flex items-center gap-2 text-cyan-400 font-bold uppercase tracking-widest text-sm">
                  ...
                  Searching for match... {queueCount > 0 ? queueCount : 1} / {MAX_PLAYERS}
                </div>
              ) : (
          ...
          <div className="max-w-3xl mx-auto bg-white/5 border border-white/10 rounded-3xl p-6 text-left">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2">
              <div className="bg-white/5 border border-cyan-500/30 p-4 rounded-2xl flex flex-col items-center min-w-[120px] relative">
                <div className="absolute -top-2 -right-2 bg-cyan-500 text-black p-1 rounded-full shadow-[0_0_10px_rgba(6,182,212,0.5)]"><Crown size={12} strokeWidth={4} /></div>
              ...
              {[1, 2, 3, 4].map((slot) => (
                <div key={slot} className="bg-white/5 border border-white/10 border-dashed p-4 rounded-2xl flex flex-col items-center justify-center min-w-[120px] opacity-50 hover:opacity-100 transition-opacity cursor-pointer">
                  <div className="w-12 h-12 bg-black/50 rounded-xl mb-3 flex items-center justify-center"><Users size={20} className="text-gray-500" /></div>
